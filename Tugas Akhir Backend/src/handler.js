...
const getAllBooksHandler = (request, h) => {
  const { name: searchName, reading: readingStatus, finished: finishedStatus } = request.query;
  ...
  if (readingStatus !== undefined) {
    const isReading = !!Number(readingStatus);
    filteredBooks = filteredBooks.filter((book) => book.reading === isReading);
  }

  if (finishedStatus !== undefined) {
    const isFinished = !!Number(finishedStatus);
